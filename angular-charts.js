	...
	function ($templateCache, $compile, $rootElement, $window, $timeout, $sce) {
		...
		function getChildrenByClassname(childrens, className) {
			...
			_.forEach(childrens, function (child) {
				//console.debug('ALPHA', className, $(child).hasClass(className), key, childrens[key]);
				jChild = [child];
			});

			//console.debug('childrens', childrens, className);
			/*
			 for (var i in childrens) {
			 console.debug('angular.isElement', childrens[i]);
			 if (angular.isElement(childrens[i])) {
			 console.debug('cond met');
			 child = angular.element(childrens[i]);
			 if (child.hasClass(className))
			 return child;
			 } else {
			 console.debug('cond not met');
			 }
			 }
			 console.debug('ALPHA ::: Angular Returning Child ', child);
			 console.debug('ALPHA ### $jQuery Returning Child ', jChild);
			 */
			return jChild;
		}
		/**
		...
		function link(scope, element, attrs) {
			...
			prepareConfig();
			var totalWidth = $(element[0]).height();
			var totalHeight = $(element[0]).height();
			...
			function setContainers() {
				...
				var acTitleContainer = getChildrenByClassname(childrens, 'ac-title')[0];
				chartContainer = getChildrenByClassname(childrens, 'ac-chart');
				legendContainer = getChildrenByClassname(childrens, 'ac-legend');

				height -= $(acTitleContainer).height(); //getChildrenByClassname(childrens, 'ac-title')[0].clientHeight;
			}
			/**
			...
			function makeToolTip(data, event) {
				...
				var el = $('<p class="ac-tooltip"></p>').html(data).css({
			...
			w.bind('resize', function (ev) {
				...
				resizePromise = $timeout(function () {
					totalWidth = $(element[0]).width(); //.clientWidth;
					totalHeight = $(element[0]).height(); //.clientHeight;
					init();
				}, 100);
			});
			scope.getWindowDimensions = function () {
				return {
					'h':$(w[0]).height(),//clientHeight
					'w':$(w[0]).width() //w[0].clientWidth
				};
			};
			...
			scope.$watch(function () {
				return {
					w: $(element[0]).width(),//.clientWidth
					h: $(element[0]).height() //.clientHeight
				};
			}, function (newvalue) {
